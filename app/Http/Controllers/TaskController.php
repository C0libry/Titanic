...
class TaskController extends Controller
{
    ...
    {
        if ($user = User::query()->where('users.username', $request->username)->first()) {
            $task = new Task();
    ...
    public function destroy($current_task_id)
    {
        ...
        $current_chat = Chat::query()->where('id', $current_task->chat_id)->first();
        if ($current_chat->creator_user_id == Auth::user()->id)
            Task::query()->where('id', $current_task_id)->delete();
